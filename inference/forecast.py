...
class ForecastEngine:
    ...
    def compute_feature_attribution(self, x_tensor: torch.Tensor) -> List[Dict[str, Any]]:
        """
        Gradient-based Feature Attribution (Saliency Map / Gradient-based Attribution) for LSTM.
        Saliency = | d(Current_Prob + Sum(Forecast_Probs)) / d(X) | * |X|
        Aggregated across the time window dimension (W=30) to compute per-feature importance scores.
        """
        ...
        grads = x_clone.grad.data.abs().cpu().numpy()[0]  # (30, num_features)
        x_val = x_tensor.detach().cpu().numpy()[0]        # (30, num_features)

        # Attribution per feature channel = mean(|grad * X|) across time window
        attributions = np.mean(grads * np.abs(x_val), axis=0)  # (num_features,)
