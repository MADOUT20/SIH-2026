...
device_service = DeviceService(packet_service, threat_service, forecast_service, mitre_service)
...
async def get_network_topology(mode: str = Query("live", description="Mode: live or demo")):
    ...
    try:
        packets = packet_service.packets
        reg_devices = device_service.get_registered_devices_list()
        topology = await traffic_service.get_network_topology(packets, mode=mode, registered_devices=reg_devices)
        return topology
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
async def get_mitre_stages(mode: str = Query("all", description="Mode: all, live, or demo")):
    ...
    if mode == "live":
        ...
        if not threat_service.manual_threats and not getattr(threat_service, "is_simulation_cleared", False):
            threat_service.simulate_attack_scenario("multi_stage")
        threat_list = threat_service.manual_threats
    else:
        if not threat_service.manual_threats and not threat_service.detected_threats and not getattr(threat_service, "is_simulation_cleared", False):
            threat_service.simulate_attack_scenario("multi_stage")
        threat_list = threat_service.threats
...
# ===== DEVICES ROUTES =====
devices_router = APIRouter(prefix="/api/devices", tags=["Device Connectivity & Malware Detection"])
...
@devices_router.get("/discover")
async def discover_devices(mode: str = Query("live", description="Mode: live or demo")):
    """
    Discover local / nearby network devices via ARP table, host network interfaces,
    and observed packet flows.
    """
    try:
        discovered = device_service.discover_local_devices(mode=mode)
        active_device = device_service.get_active_connected_device(mode=mode)
        return {
            "status": "success",
            "mode": mode,
        ...
        logger.error(f"Error during device discovery ({mode}): {e}")
        return {
            "status": "error",
            "error_type": "DISCOVERY_UNAVAILABLE",
            "message": f"Local network discovery unavailable: {str(e)}",
            "mode": mode,
            "count": 0,
            "devices": [],
...
@devices_router.get("/active")
async def get_active_device(mode: str = Query("live", description="Mode: live or demo")):
    """Get the currently connected active monitoring device."""
    try:
        active_dev = device_service.get_active_connected_device(mode=mode)
        return {
            "status": "success",
            "mode": mode,
...
@devices_router.post("/connect")
async def connect_device(req: DeviceConnectRequest):
    """
    Connect to a nearby/local network device with explicit user permission.
    Sets this device as the primary active monitoring target for NetGuard.
    """
    try:
        dev = device_service.connect_device(
            device_id=req.device_id or f"dev_{req.ip.replace('.', '_')}",
            ip=req.ip,
            name=req.name,
            mac=req.mac,
            device_type=req.device_type or "Laptop",
        ...
        return {
            "status": "success",
            "message": f"Connected to '{dev['name']}' ({dev['ip']}) for active monitoring.",
            "device": dev,
        }
...
@devices_router.post("/disconnect")
async def disconnect_device():
    """Disconnect active device monitoring target."""
    try:
        device_service.disconnect_device()
        return {
            "status": "success",
            "message": "Device disconnected. Monitoring returned to general network scope.",
        }
    except Exception as e:
        raise HTTPException(status_code=500, detail=str(e))
...
@devices_router.get("")
async def list_devices(mode: str = Query("live", description="Mode: live or demo")):
    """List all registered and observed devices with connectivity and security status."""
    try:
        devices = device_service.get_all_devices(mode=mode)
        active_device = device_service.get_active_connected_device(mode=mode)
        return {
            "status": "success",
            "mode": mode,
...
@devices_router.post("")
async def add_device(req: DeviceRegisterRequest):
    """Register/connect a device into the connectivity and threat monitoring system."""
    try:
        dev = device_service.connect_device(
            device_id=f"dev_{req.ip.replace('.', '_')}",
            name=req.name,
            ip=req.ip,
            mac=req.mac,
        ...
        return {
            "status": "success",
            "message": f"Device '{dev['name']}' connected successfully",
            "device": dev,
        }
    except ValueError as ve:
        raise HTTPException(status_code=400, detail=str(ve))
...
async def get_device_details(device_id: str, mode: str = Query("live", description="Mode: live or demo")):
    """Get connectivity, traffic summary, and security state for a specific device."""
    try:
        dev = device_service.get_registered_device(device_id)
        target_ip = dev["ip"] if dev else device_id
        target_mac = dev.get("mac") if dev else None

        conn = device_service.resolve_device_connectivity(target_ip, target_mac, mode=mode)
        assessment = device_service.assess_device_malware_threat(target_ip, target_mac, mode=mode)

        return {
            "id": dev["id"] if dev else f"dev_{target_ip.replace('.', '_')}",
            "name": dev["name"] if dev else f"Device ({target_ip})",
            "ip": target_ip,
            "mac": target_mac,
            "device_type": dev.get("device_type", "Endpoint") if dev else "Endpoint",
            "description": dev.get("description") if dev else None,
            "is_registered": dev is not None,
            "connectivity": conn,
            "security": assessment,
            "timestamp": datetime.now().isoformat(),
        }
...
@devices_router.get("/{device_id}/traffic")
async def get_device_traffic(device_id: str, mode: str = Query("live", description="Mode: live or demo")):
    """Get observed network traffic, protocols, and destination endpoints for a device."""
    try:
        dev = device_service.get_registered_device(device_id)
        target_ip = dev["ip"] if dev else device_id
        target_mac = dev.get("mac") if dev else None

        traffic_info = device_service.get_device_traffic_details(target_ip, target_mac, mode=mode)
        return {
            "status": "success",
            "device_id": device_id,
            "device_name": dev["name"] if dev else target_ip,
            "mode": mode,
            **traffic_info,
        }
    except Exception as e:
...
@devices_router.get("/{device_id}/threat-assessment")
async def get_device_threat_assessment(device_id: str, mode: str = Query("live", description="Mode: live or demo")):
    """Run evidence-grounded malware and threat assessment on device traffic using LSTM World Model & MITRE."""
    try:
        dev = device_service.get_registered_device(device_id)
        target_ip = dev["ip"] if dev else device_id
        target_mac = dev.get("mac") if dev else None
