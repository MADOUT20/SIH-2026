...
class ThreatDetectionService:
    def __init__(self):
        ...
        self.is_simulation_cleared: bool = False
        ...
        if action == "BLOCK":
            ...
            if blocked_domain:
                self.blocked_domains[blocked_domain] = {
                    "blocked_at": now,
                    "reason": threat.get("type") or "Active Threat Detection",
                    "threat_id": threat_id,
                    "threat_type": threat.get("type"),
                    "severity": threat.get("severity", "HIGH"),
                    "destination_host": threat.get("destination_host") or blocked_domain,
                    "source_ip": threat.get("source_ip"),
                    "description": threat.get("description"),
                    "evidence": threat.get("evidence", []),
                    "is_demo": threat.get("is_demo", False),
                }
    ...
    def get_blocked_domains(self, mode: str = "all") -> List[Dict[str, Any]]:
        ...
        for domain, details in sorted(
            ...
            is_demo = details.get("is_demo", False)
            ...
            blocked_entries.append(
                {
                    ...
                    "destination_host": details.get("destination_host", domain),
    ...
    def unblock_domain(self, domain: str) -> Dict[str, Any]:
        ...
        now = datetime.now().isoformat()
        threat_id_info = f" (associated threat: {removed.get('threat_id')})" if removed.get("threat_id") else ""
        self._add_notification(
            title="Website & Threat Unblocked",
            message=f"Website unblock applied to {normalized_domain}{threat_id_info}",
            severity="LOW",
            notification_type="RESPONSE_ACTION",
        )
        ...
        return {
            ...
            "threat_id": removed.get("threat_id"),
            "timestamp": now,
            "verified_unblocked": not self.is_domain_blocked(normalized_domain),
        }

    def unlock_threat(self, threat_id: str) -> Dict[str, Any]:
        """Directly unlock a threat, remove active block rules, and revert status to active."""
        target_threat = None
        for t in self.detected_threats + self.manual_threats:
            if t.get("id") == threat_id:
                target_threat = t
                break

        if not target_threat:
            return {"success": False, "error": f"Threat {threat_id} not found"}

        blocked_domain = self._domain_to_block_for_threat(target_threat)
        source_ip = target_threat.get("source_ip")

        if blocked_domain:
            normalized_domain = self._normalize_dns_query(blocked_domain)
            if normalized_domain in self.blocked_domains:
                self.blocked_domains.pop(normalized_domain, None)
            self._release_domain_block_state(normalized_domain)

        if source_ip and source_ip in self.blocked_ips:
            self.blocked_ips.pop(source_ip, None)

        target_threat["status"] = "active"
        target_threat.pop("action_taken", None)
        target_threat.pop("response_time", None)

        now = datetime.now().isoformat()
        self._add_notification(
            title="Threat Unlocked",
            message=f"Threat {threat_id} ({target_threat.get('type')}) unlocked. Active firewall/proxy block removed.",
            severity="LOW",
            notification_type="RESPONSE_ACTION",
        )
        ...
        return {
            "success": True,
            "message": f"Threat {threat_id} unlocked successfully",
            "threat_id": threat_id,
            "domain": blocked_domain,
            "status": "active",
            "timestamp": now,
            "verified_unblocked": not self.is_domain_blocked(blocked_domain) if blocked_domain else True,
        }
    ...
    def _build_threat(
        ...
        threat = {
            ...
            "is_demo": is_demo,
            "origin": "DEMO / SEEDED DATA" if is_demo else "LIVE CAPTURE DETECTION",
    ...
    def _deduplicate_threats(self, threats: List[Dict]) -> List[Dict]:
        ...
        for threat in threats:
            ...
            if threat_id and threat_id in seen_ids:
                continue
    ...
    async def scan_url(self, url: str) -> Dict[str, Any]:
        ...
        if scan_result.get("is_malicious"):
            ...
            self.detected_threats.insert(0, new_threat)
            self.detected_threats = self.detected_threats[:self.max_manual_threats]
    ...
    def simulate_attack_scenario(self, scenario_type: str = "multi_stage") -> List[Dict[str, Any]]:
        ...
        """
        self.is_simulation_cleared = False
        now = datetime.now()
    ...
    def clear_simulated_threats(self) -> int:
        ...
        self.is_simulation_cleared = True
        return count
