    ...
    def build_attack_chain_summary(threats: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        for threat in threats:
            ...
            if stage_num in stages_hit:
                ...
                # Deduplicate threat instances within the stage
                already_present = any(
                    (t.get("id") and t.get("id") == threat_id) or
                    (t.get("type") == threat_type and t.get("source_ip") == threat.get("source_ip") and t.get("destination_host") == threat.get("destination_host"))
                    for t in stages_hit[stage_num]["threats"]
                )
