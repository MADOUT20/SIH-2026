...
class ForecastService:
    ...
    def explain_packet_prediction(self, packet_data: Dict[str, Any]) -> Dict[str, Any]:
        """
        Computes 27-feature flow representation and LSTM model gradient-based feature attribution
        for an individual packet or flow.
        """
        friendly_names = {
            ...
            "flow_iat_std": "IAT Standard Deviation",
            ...
            "protocol_tcp": "Protocol TCP Indicator",
            "protocol_udp": "Protocol UDP Indicator",
            "is_high_risk_port": "High Risk Port Indicator",
            "fwd_bwd_bytes_ratio": "Forward/Backward Bytes Ratio",
        }

        # Extract the exact 27 canonical features for this packet/flow
        features = extract_27_flow_features([packet_data], self.interval_seconds)

        # Construct a 30-state sequence using current state history if available, or tiled states
        if len(self.state_history) >= self.window_size:
            seq = list(self.state_history)[:-1] + [features]
        elif len(self.state_history) > 0:
            seq = list(self.state_history) + [features] * (self.window_size - len(self.state_history))
        else:
            seq = [features] * self.window_size

        seq_np = np.array(seq, dtype=np.float32)
        forecast_res = self.engine.forecast(seq_np)

        raw_top = forecast_res.get("top_features", [])
        formatted_features = []
        for rank, tf in enumerate(raw_top[:5], start=1):
            fname = tf.get("feature", "")
            dname = friendly_names.get(fname, fname.replace("_", " ").title())
            formatted_features.append({
                "rank": rank,
                "feature": fname,
                "display_name": dname,
                "importance": tf.get("importance", 0.0),
                "contribution_percent": f"{round(tf.get('importance', 0.0) * 100, 1)}%",
                "direction": tf.get("direction", "+"),
                "risk_impact": "Increases Risk" if tf.get("direction", "+") == "+" else "Decreases Risk",
                "raw_value": tf.get("raw_value", 0.0),
                "description": tf.get("description", ""),
            })

        return {
            "status": "success",
            "explainability_model": "Temporal LSTM World Model (27-Feature Architecture)",
            "method": "Gradient-Based Feature Attribution (Integrated Gradients / Saliency x Input)",
            "predicted_stage": forecast_res.get("predicted_stage", "Normal / Benign"),
            "current_probability": forecast_res.get("current_probability", 0.0),
            "stage_confidence": forecast_res.get("stage_confidence", 0.0),
            "top_contributing_features": formatted_features,
            "extracted_features_count": 27,
            "flow_snapshot": {
                "source_ip": packet_data.get("source_ip"),
                "dest_ip": packet_data.get("dest_ip"),
                "dest_port": packet_data.get("dest_port"),
                "protocol": packet_data.get("protocol"),
                "size_bytes": packet_data.get("size_bytes"),
            },
        }
