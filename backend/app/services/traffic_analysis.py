    ...
    async def get_network_topology(
        ...
        if mode == "demo":
            ...
            if registered_devices:
                existing_node_ips = {n["ip"] for n in nodes}
                seen_reg_ips = set()
                for reg in registered_devices:
                    r_ip = reg.get("ip")
                    if not r_ip or r_ip in seen_reg_ips:
                        continue
                    seen_reg_ips.add(r_ip)
                    if r_ip in existing_node_ips:
                        for n in nodes:
                            if n["ip"] == r_ip:
                                n["is_registered"] = True
                                n["label"] = f"{reg.get('name', n['label'])}"
                                if reg.get("device_type"):
                                    n["type"] = reg["device_type"]
                    else:
                        nodes.append({
                            "id": r_ip,
                            "ip": r_ip,
                            "label": reg.get("name", f"Device ({r_ip})"),
                            "role": reg.get("device_type", "workstation").lower(),
                            "type": reg.get("device_type", "Endpoint"),
                            "status": "not_observed",
                            "packets_in": 0,
                            "packets_out": 0,
                            "total_bytes": 0,
        ...
        if not packets:
            ...
            if registered_devices:
                for reg in registered_devices:
                    r_ip = reg.get("ip")
                    if not r_ip or r_ip in seen_ips:
                        continue
                    seen_ips.add(r_ip)
                    reg_nodes.append({
                        "id": r_ip,
            ...
            return {
                ...
                "total_devices": len(reg_nodes),
                "total_connections": 0,
                "nodes": reg_nodes,
                "links": [],
                "timestamp": now_str,
                "message": "Registered devices ready. Start traffic capture to observe packet flows." if reg_nodes else "No live packets captured yet. Start traffic capture to discover network devices.",
            }
        ...
        reg_by_ip = {d["ip"]: d for d in (registered_devices or []) if d.get("ip")}
        nodes = []
        for ip, stats in device_stats.items():
            ...
            reg_info = reg_by_ip.get(ip) if "reg_by_ip" in locals() else None
            is_reg = reg_info is not None
            if reg_info:
                label = reg_info.get("name", label)
                device_type = reg_info.get("device_type", device_type)
        ...
        # Append unobserved registered devices
        node_ips = {n["ip"] for n in nodes}
        seen_unobserved_ips = set()
        if registered_devices:
            for reg in registered_devices:
                r_ip = reg.get("ip")
                if r_ip and r_ip not in node_ips and r_ip not in seen_unobserved_ips:
                    seen_unobserved_ips.add(r_ip)
                    nodes.append({
                        "id": r_ip,
                        "ip": r_ip,
                        "label": reg.get("name", f"Device ({r_ip})"),
        ...
        for flow_key, fstats in flow_map.items():
            ...
            if src not in active_node_ids or dst not in active_node_ids:
                continue
            if flow_key in seen_link_ids:
                continue
            seen_link_ids.add(flow_key)
