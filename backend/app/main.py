...
from app.api.routes import (
    ...
    mitre_router, ml_router, forecast_router, proxy_service
)
...
async def startup_event():
    import asyncio
    from app.api.routes import packet_service

    # Check if continuous packet capture should autostart via environment flag
    auto_capture = os.getenv("CAPTURE_AUTOSTART", "0").lower() in {"1", "true", "yes", "on"}
    if auto_capture:
        packet_service.start_background_capture()
