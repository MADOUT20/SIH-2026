...
async function apiRequest<T>(path: string, init: RequestInit = {}): Promise<T> {
  ...
  try {
    ...
    const msg = error instanceof Error ? error.message : "Unable to reach the backend right now."
...
// ===== DEVICE CONNECTIVITY & THREAT MONITORING ENDPOINTS =====
...
export interface NetworkDevice {
  ...
  is_registered?: boolean
  connectivity_status?: "CONNECTED" | "ACTIVE" | "INACTIVE" | "NOT OBSERVED" | string
  has_traffic?: boolean
  packet_count?: number
  packets_in?: number
  packets_out?: number
  ...
  security_status?: "SECURE" | "LOW RISK" | "SUSPICIOUS" | "HIGH RISK" | "MALICIOUS / THREAT DETECTED" | "NOT ASSESSED" | string
...
export type DeviceItem = NetworkDevice

export function normalizeDevice(dev: any): NetworkDevice | null {
  if (!dev || typeof dev !== "object") return null
  const rawIp = dev.ip || dev.ip_address || ""
  const rawMac = dev.mac || dev.mac_address || null
  const cleanIp = typeof rawIp === "string" ? rawIp.trim() : (rawIp ? String(rawIp).trim() : "")
  const cleanMac = typeof rawMac === "string" && rawMac.trim() ? rawMac.trim().toUpperCase() : (rawMac ? String(rawMac).trim().toUpperCase() : null)
  const canonicalId = cleanIp ? `dev_${cleanIp.replace(/[.:]/g, "_")}` : (dev.id || `dev_${Date.now()}`)

  return {
    ...dev,
    id: typeof dev.id === "string" && dev.id.trim() ? dev.id.trim() : canonicalId,
    name: typeof dev.name === "string" && dev.name.trim() ? dev.name.trim() : (cleanIp ? `Device (${cleanIp})` : "Network Endpoint"),
    ip: cleanIp,
    ip_address: cleanIp,
    mac: cleanMac,
    mac_address: cleanMac,
    device_type: dev.device_type || (dev.is_gateway ? "Gateway" : "Endpoint"),
    connectivity_status: dev.connectivity_status || (dev.has_traffic || (dev.packet_count || 0) > 0 ? "CONNECTED" : "NOT OBSERVED"),
    has_traffic: Boolean(dev.has_traffic || (dev.packet_count || 0) > 0),
    packet_count: Number(dev.packet_count || 0),
    packets_in: Number(dev.packets_in || 0),
    packets_out: Number(dev.packets_out || 0),
    total_bytes: Number(dev.total_bytes || 0),
    last_seen: dev.last_seen || null,
    security_status: dev.security_status || ((dev.packet_count || 0) > 0 ? "SECURE" : "NOT ASSESSED"),
    attack_probability: Number(dev.attack_probability || 0),
    mitre_stage: dev.mitre_stage || null,
    is_registered: Boolean(dev.is_registered),
    is_demo: Boolean(dev.is_demo),
    is_local_host: Boolean(dev.is_local_host),
    is_gateway: Boolean(dev.is_gateway),
    can_monitor: dev.can_monitor !== false,
    consent_required: dev.consent_required !== false,
  }
}

export interface DeviceTrafficDetails {
  status: string
  device_id: string
  device_name: string
  mode: string
...
export interface DeviceThreatAssessment {
  status: string
  device_id: string
  device_name: string
  mode: string
  ...
  evidence: string[]
  explanation: string
  model_evaluated: boolean
  is_demo: boolean
}

export interface DiscoveredDevice extends NetworkDevice {}

export async function discoverDevices(mode = "live"): Promise<{
  status: string
  mode: string
  count: number
  ...
  const res: any = await apiRequest(buildUrl("/api/devices/discover", { mode }))
  return {
    ...res,
    devices: (res?.devices || []).map((d: any) => normalizeDevice(d)!),
    active_device: normalizeDevice(res?.active_device),
  }
}

export async function getActiveDevice(mode = "live"): Promise<{
  status: string
  mode: string
  has_active_device: boolean
  device: NetworkDevice | null
}> {
  const res: any = await apiRequest(buildUrl("/api/devices/active", { mode }))
  return {
    ...res,
    has_active_device: Boolean(res?.has_active_device && res?.device),
    device: normalizeDevice(res?.device),
  }
}
...
export async function connectDevice(data: {
  ...
  consent: boolean
  mode?: string
}): Promise<{ status: string; message: string; device: any }> {
  return apiRequest("/api/devices/connect", {
    method: "POST",
    body: JSON.stringify(data),
  })
}

export async function disconnectActiveDevice(): Promise<{ status: string; message: string }> {
  return apiRequest("/api/devices/disconnect", {
    method: "POST",
  })
}

export async function getDevices(mode = "live"): Promise<{ status: string; mode: string; count: number; devices: NetworkDevice[] }> {
  const res: any = await apiRequest(buildUrl("/api/devices", { mode }))
  return {
    ...res,
    devices: (res?.devices || []).map((d: any) => normalizeDevice(d)!),
  }
...
export async function getDeviceDetails(deviceId: string, mode = "live"): Promise<any> {
  return apiRequest(buildUrl(`/api/devices/${encodeURIComponent(deviceId)}`, { mode }))
}

export async function getDeviceTraffic(deviceId: string, mode = "live"): Promise<DeviceTrafficDetails> {
  return apiRequest(buildUrl(`/api/devices/${encodeURIComponent(deviceId)}/traffic`, { mode }))
}

export async function getDeviceThreatAssessment(deviceId: string, mode = "live"): Promise<DeviceThreatAssessment> {
  return apiRequest(buildUrl(`/api/devices/${encodeURIComponent(deviceId)}/threat-assessment`, { mode }))
}
