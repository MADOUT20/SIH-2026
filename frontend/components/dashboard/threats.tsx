            ...
            <div className="rounded-lg bg-muted p-3 text-xs space-y-2">
              ...
              {threat.evidence && threat.evidence.length > 0 && (
                <ul className="list-disc list-inside space-y-1 text-muted-foreground pt-1 border-t border-border">
                  {Array.from(new Set(threat.evidence)).map((ev, i) => (
                    <li key={createEntityKey("modal-ev", threat.id, [String(i), ev])}>{ev}</li>
                  ))}
                </ul>
              )}
            </div>
...
export function ThreatDetectionPanel({ excludeLow = false }: ThreatDetectionPanelProps) {
  ...
  const fetchThreats = async () => {
    try {
      ...
      const rawThreats = (data.threats || []).filter((threat) => threat.status !== "ignored")
      setThreats(
        deduplicateBy(
          rawThreats,
          (t, i) => t.id || `${t.type}-${t.source_ip || "src"}-${t.destination_host || "dst"}-${i}`,
          "ThreatDetectionPanel.threats"
        )
      )
      setError("")
    } catch (err: any) {
      console.warn("Threats telemetry unavailable:", err instanceof Error ? err.message : String(err))
              ...
              >
                ...
                {activeThreat.evidence && activeThreat.evidence.length > 0 && (
                  <div className="mb-3 mt-3 space-y-1">
                    {Array.from(new Set(activeThreat.evidence)).slice(0, 2).map((item, index) => (
                      <p key={createEntityKey("active-ev", activeThreat.id, [String(index), item])} className="text-xs text-slate-500">
                        • {item}
                      </p>
                    ))}
                  </div>
...
export function ObservedDevicesCard() {
  const { connectedDevice, discoveredDevices, openConnectModal, disconnectDevice, connectDevice } = useDevice()
    ...
    <Card className="border-border bg-card">
      <CardHeader className="flex flex-row items-center justify-between gap-3 pb-3">
        ...
        <Button
          ...
          className="h-7 text-xs px-2.5 font-semibold gap-1 text-indigo-600 hover:text-indigo-700 dark:text-indigo-400"
        >
      ...
      <CardContent className="space-y-3">
        {/* Active Connected Device Callout */}
        {connectedDevice ? (
          <div className="rounded-xl border border-emerald-500/40 bg-emerald-500/10 p-3.5 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Badge className="h-4 bg-emerald-600 text-[9px] font-bold text-white px-1.5">
                ● CONNECTED
              </Badge>
            </div>
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="text-xs text-muted-foreground font-mono">
                  {connectedDevice.ip} • {connectedDevice.device_type || "Endpoint"}
                </p>
              </div>
              <Button
                size="sm"
                ...
                className="h-6 text-[11px] px-2 text-rose-500 hover:text-rose-600 hover:bg-rose-50 dark:hover:bg-rose-950/40"
              >
                Disconnect
              </Button>
            </div>
          ...
          <div className="rounded-xl border border-dashed border-slate-300 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/30 p-3.5 text-center space-y-2">
            <p className="text-xs text-slate-600 dark:text-slate-400 font-medium">
              No device actively connected for scoped monitoring.
            </p>
            <Button
              size="sm"
              onClick={openConnectModal}
              className="h-7 text-xs bg-indigo-600 hover:bg-indigo-700 text-white font-semibold gap-1 shadow-sm"
            >
              <Wifi className="h-3 w-3" />
          ...
          <div className="space-y-1.5 pt-1">
            ...
            <div className="max-h-40 overflow-y-auto space-y-1.5 pr-1">
              {deduplicateBy(
                discoveredDevices,
                (d) => d.id || d.ip,
                "ObservedDevicesCard.devices"
              ).slice(0, 5).map((dev) => {
                const isSelected = connectedDevice?.ip === dev.ip || connectedDevice?.id === dev.id
                return (
                  <div
                    key={createEntityKey("discovered-dev", dev.id, [dev.ip])}
                    className="flex items-center justify-between rounded-lg border border-border/80 bg-background/50 p-2 text-xs"
                  >
                    <div className="min-w-0 pr-2">
                      <p className="truncate font-medium text-foreground">{dev.name}</p>
                      <p className="text-[10px] text-muted-foreground font-mono">{dev.ip}</p>
                    </div>
                    {isSelected ? (
                      <Badge className="h-4 bg-emerald-600 text-[9px] text-white">ACTIVE</Badge>
                    ) : (
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => connectDevice(dev, true)}
                        className="h-6 text-[10px] px-2 text-indigo-600 hover:text-indigo-700 hover:bg-indigo-50 dark:text-indigo-400 font-semibold"
                      >
                        Connect
                      </Button>
                    )}
...
export function BlockedSitesCard() {
  ...
  const fetchBlocked = async () => {
    try {
      ...
      console.warn("Blocked sites unavailable:", err instanceof Error ? err.message : String(err))
      setBlockedSites([])
    } finally {
      setLoading(false)
    }
      ...
      <CardContent className="space-y-3">
        ...
        {loading ? (
          ...
          <div className="space-y-2.5 max-h-[340px] overflow-y-auto">
            {deduplicateBy(
              blockedSites,
              (s, i) => s.threat_id || s.domain || `blocked-${i}`,
              "BlockedSitesCard.blockedSites"
            ).map((site) => {
              const key = createEntityKey("blocked-site", site.threat_id, [site.domain])
