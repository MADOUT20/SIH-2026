            ...
            {taxonomy?.techniques && (
              <div className="border-t border-slate-100 dark:border-slate-800 pt-3">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2 flex items-center gap-1.5">
                  ...
                  Standard MITRE ATT&CK Techniques & Defenses for {selectedStage.tactic_name}
                </h4>
                ...
                <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(taxonomy.techniques)
                    ...
                    .map(([key, tech]: [string, any]) => (
                      <div 
                        key={key}
                        className="p-3 rounded-lg border border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900 space-y-1 text-xs"
                      >
                        <div className="flex items-center justify-between">
                        ...
                        {tech.mitigation && (
                          <div className="mt-1.5 pt-1.5 border-t border-slate-100 dark:border-slate-800 text-[10px] text-emerald-700 dark:text-emerald-400">
                            <strong>Mitigation:</strong> {tech.mitigation}
                          </div>
                        )}
                      </div>
                    ))}
