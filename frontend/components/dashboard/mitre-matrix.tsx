...
export function MitreAttackMatrix() {
  ...
  const fetchData = async () => {
    try {
      ...
      setError("Failed to load MITRE ATT&CK matrix. Backend service may be initializing.")
    } finally {
      setLoading(false)
    }
  }
  ...
  const activeStages = attackChain?.progression || []
  ...
  const currentSelectedStage = useMemo(() => {
    ...
    if (selectedStageNumber !== null) {
      const match = allStages.find((s) => s.stage_number === selectedStageNumber)
      if (match) return match
    }
    return activeStages.length > 0 ? activeStages[0] : allStages[0] || null
  }, [allStages, activeStages, selectedStageNumber])
  ...
  const handleSimulateAttack = async (type: "multi_stage" | "trojan" = "multi_stage") => {
    try {
      ...
      if (res.attack_chain.progression && res.attack_chain.progression.length > 0) {
        setSelectedStageNumber(res.attack_chain.progression[0].stage_number)
      } else if (res.attack_chain.all_stages && res.attack_chain.all_stages.length > 0) {
        setSelectedStageNumber(res.attack_chain.all_stages[0].stage_number)
      }
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-7 gap-2">
          {allStages.map((stage) => {
            const isSelected = currentSelectedStage?.stage_number === stage.stage_number
            ...
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div className="space-y-1">
                <div className="flex items-center gap-2">
                  <Badge 
                    style={{ backgroundColor: currentSelectedStage.color, color: "#ffffff" }}
                    className="font-bold text-xs"
                  >
                    Stage {currentSelectedStage.stage_number}
                  </Badge>
                  <span className="text-xs font-mono text-slate-500">[{currentSelectedStage.tactic_id}]</span>
                  {currentSelectedStage.count > 0 && (
                    <Badge variant="destructive" className="text-xs">
                      {currentSelectedStage.count} Active Finding{currentSelectedStage.count === 1 ? "" : "s"}
                    </Badge>
                  )}
                </div>
                <CardTitle className="text-lg font-bold text-slate-900 dark:text-slate-100">
                  {currentSelectedStage.tactic_name}
                </CardTitle>
                <CardDescription className="text-xs sm:text-sm text-slate-600 dark:text-slate-400">
                  {currentSelectedStage.description || "MITRE ATT&CK tactical kill-chain stage."}
                </CardDescription>
              </div>

              <a
                href={`https://attack.mitre.org/tactics/${currentSelectedStage.tactic_id}/`}
                target="_blank"
                rel="noreferrer"
          ...
          <CardContent className="pt-4 space-y-4">
            ...
            <div>
              <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2 flex items-center gap-1.5">
                ...
                Active Threats Mapped to this Stage ({currentSelectedStage.threats.length})
              </h4>

              {currentSelectedStage.threats.length === 0 ? (
                <div className="p-4 rounded-xl bg-slate-50 dark:bg-slate-900/60 border border-dashed border-slate-200 dark:border-slate-800 text-center">
                  ...
                  <p className="text-xs font-medium text-slate-700 dark:text-slate-300">
                    No active threats currently observed at Stage {currentSelectedStage.stage_number} ({currentSelectedStage.tactic_name}).
                  </p>
                ...
                <div className="grid gap-2 sm:grid-cols-2">
                  {deduplicateBy(
                    currentSelectedStage.threats || [],
                    (t, i) => t.id || `${t.type}-${t.technique_id}-${t.source_ip || "src"}-${t.destination_host || "dst"}-${i}`,
                    "MitreAttackMatrix.threats"
                  ).map((threat) => (
                    <div 
                      key={createEntityKey("mitre-threat", threat.id, [threat.type, threat.technique_id, threat.source_ip, threat.destination_host])}
                    ...
                    >
                      <div className="flex items-center justify-between">
                        <span className="text-xs font-bold text-slate-900 dark:text-slate-100">
                          {threat.type ? String(threat.type).replace(/_/g, " ") : "OBSERVED ANOMALY"}
                        </span>
                        <Badge 
                      ...
                      <div className="text-xs text-slate-600 dark:text-slate-400 flex items-center gap-1.5 font-mono">
                        <span className="font-semibold text-indigo-600 dark:text-indigo-400">
                          {threat.technique_id || "T1046"}
                        </span>
                        <span>•</span>
                        <span>{threat.technique_name || "Network Finding"}</span>
                      </div>
            ...
            {taxonomy?.techniques && (
              <div className="border-t border-slate-100 dark:border-slate-800 pt-3">
                <h4 className="text-xs font-semibold uppercase tracking-wider text-slate-500 mb-2 flex items-center gap-1.5">
                  ...
                  Standard MITRE ATT&CK Techniques, Detection Logic & Mitigations for {currentSelectedStage.tactic_name}
                </h4>

                <div className="grid gap-2.5 sm:grid-cols-2 lg:grid-cols-3">
                  {Object.entries(taxonomy.techniques)
                    .filter(([_, t]: [string, any]) => t.stage_number === currentSelectedStage.stage_number || t.tactic_id === currentSelectedStage.tactic_id)
