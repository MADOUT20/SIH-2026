...
const navItems = [
  ...
  { icon: BrainCircuit, label: "Benchmarks & Scores", id: "ml" },
...
export function DashboardSidebar({
  ...
  const navigationItems = (
    <nav className="flex-1 space-y-1 p-3 overflow-y-auto overflow-x-hidden min-h-0">
      {navItems.map((item) => {
  ...
  return (
    <TooltipProvider delayDuration={0}>
      <aside
        className={cn(
          "flex flex-col border-r border-sidebar-border bg-sidebar transition-all duration-200 h-full shrink-0 select-none",
          collapsed ? "w-16" : "w-60"
        )}
      >
        {/* Logo */}
