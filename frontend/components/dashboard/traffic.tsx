      ...
      <CardContent>
        <div className="space-y-3">
          {loading ? (
            ...
            trafficData.map((traffic, i) => (
              <div
                key={createEntityKey("traffic-proto", traffic.protocol, [String(i)])}
                className="flex items-center justify-between rounded-lg border border-slate-200/60 bg-slate-50/80 p-2.5 dark:border-slate-800 dark:bg-slate-850"
              >
                <span className="font-medium text-slate-900 dark:text-slate-100">{traffic.protocol}</span>
                <div className="text-right">
                  <p className="text-sm font-semibold font-mono text-slate-900 dark:text-slate-100">{traffic.packets.toLocaleString()} packets</p>
...
export function TrafficChartPanel() {
  ...
  const fetchStats = async () => {
    try {
      ...
      console.warn("Traffic telemetry unavailable:", err instanceof Error ? err.message : String(err))
  ...
  return (
    <Card>
          ...
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {connectedDevice && (
              <Badge variant="outline" className="bg-indigo-500/10 text-indigo-600 dark:text-indigo-400 border-indigo-500/30 text-[10px] font-bold">
                ● TARGET: {connectedDevice.name} ({connectedDevice.ip || connectedDevice.ip_address || "No IP"})
              </Badge>
            )}
          </div>
          {captureStatus && (
      ...
      <CardContent>
        {loading ? (
          <div className="flex h-48 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-900/60">
          ...
          <div className="space-y-4">
            ...
            {lastCapture && !error && (
              <div className="rounded border border-slate-200 bg-slate-50 px-3 py-2 text-sm text-slate-700 dark:border-slate-800 dark:bg-slate-800/60 dark:text-slate-300">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-2xl bg-blue-50/80 p-4 border border-blue-200/60 dark:border-blue-900/40 dark:bg-blue-950/30">
                <p className="text-sm text-slate-600 dark:text-slate-400">Total Packets</p>
                <p className="text-2xl font-bold text-blue-600 dark:text-blue-400">
                  {displayedPacketCount.toLocaleString()}
                </p>
              </div>
              <div className="rounded-2xl bg-green-50/80 p-4 border border-emerald-200/60 dark:border-emerald-900/40 dark:bg-emerald-950/30">
                <p className="text-sm text-slate-600 dark:text-slate-400">Total Bytes</p>
                <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400">
                  {formatBytes(stats?.total_bytes || 0)}
                </p>
              </div>
              <div className="rounded-2xl bg-purple-50/80 p-4 border border-purple-200/60 dark:border-purple-900/40 dark:bg-purple-950/30">
                <p className="text-sm text-slate-600 dark:text-slate-400">Avg Packet Size</p>
                <p className="text-2xl font-bold text-purple-600 dark:text-purple-400">
                  {(summary?.average_packet_size || stats?.average_packet_size || 0).toFixed(0)} B
                </p>
              </div>
              <div className="rounded-2xl bg-orange-50/80 p-4 border border-amber-200/60 dark:border-amber-900/40 dark:bg-amber-950/30">
                <p className="text-sm text-slate-600 dark:text-slate-400">Packets/sec</p>
...
export function PacketInspectionPanel() {
  ...
  const [onlyConnectedDevice, setOnlyConnectedDevice] = useState(true)
  ...
  useEffect(() => {
    const fetchPackets = async () => {
      try {
        const data = await getPackets(50, 0, currentMode)
        setPackets(data.packets || [])
      } catch (err) {
        console.warn("Packets unavailable:", err instanceof Error ? err.message : String(err))
      } finally {
        setLoading(false)
      }
    }
  ...
  const { filteredPackets, deviceFilterState } = useMemo(() => {
    let list = Array.isArray(packets) ? packets : []
    let filterState: "all" | "device_filtered" | "no_identity" = "all"

    if (connectedDevice && onlyConnectedDevice) {
      const devIp = String(connectedDevice.ip || connectedDevice.ip_address || "").trim().toLowerCase()
      const devMac = String(connectedDevice.mac || connectedDevice.mac_address || "").trim().toLowerCase()

      if (!devIp && !devMac) {
        return {
          filteredPackets: [],
          deviceFilterState: "no_identity" as const,
        }
      }

      list = list.filter((p) => {
        if (!p) return false
        const srcIp = String(p.source_ip || "").trim().toLowerCase()
        const dstIp = String(p.dest_ip || "").trim().toLowerCase()
        const srcMac = String((p as any)?.source_mac || "").trim().toLowerCase()
        const dstMac = String((p as any)?.dest_mac || "").trim().toLowerCase()
        return (
          (devIp !== "" && (srcIp === devIp || dstIp === devIp)) ||
          (devMac !== "" && (srcMac === devMac || dstMac === devMac))
        )
      })
      filterState = "device_filtered"
    }
    ...
    if (searchFilter && searchFilter.trim()) {
      ...
      list = list.filter(
        (p) =>
          p &&
          ((p.source_ip && String(p.source_ip).toLowerCase().includes(q)) ||
          (p.dest_ip && String(p.dest_ip).toLowerCase().includes(q)) ||
          (p.protocol && String(p.protocol).toLowerCase().includes(q)) ||
          (p.dns_query && String(p.dns_query).toLowerCase().includes(q)) ||
          (p.observed_host && String(p.observed_host).toLowerCase().includes(q)) ||
          (p.source_port != null && String(p.source_port).includes(q)) ||
          (p.dest_port != null && String(p.dest_port).includes(q)))
      )
    }

    return { filteredPackets: list, deviceFilterState: filterState }
  }, [packets, searchFilter, connectedDevice, onlyConnectedDevice])
      ...
      <Card className="border-slate-200 bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
        <CardHeader className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between pb-3">
            ...
            <div className="flex items-center gap-2 flex-wrap">
              ...
              {connectedDevice && (
                <Badge
                  ...
                  onClick={() => setOnlyConnectedDevice(!onlyConnectedDevice)}
                  title="Click to toggle device filter"
                >
                  {onlyConnectedDevice ? `● Scoped: ${connectedDevice.name}` : "Showing All Devices"}
                </Badge>
              )}
            </div>
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">
              {connectedDevice && onlyConnectedDevice
                ? `Monitoring traffic for ${connectedDevice.name} (${connectedDevice.ip || connectedDevice.ip_address || "No IP"})`
          ...
          <div className="flex items-center gap-2 w-full sm:w-auto">
            {connectedDevice && (
              <Button
                variant={onlyConnectedDevice ? "secondary" : "outline"}
                size="sm"
                className="h-8 text-xs shrink-0"
                onClick={() => setOnlyConnectedDevice(!onlyConnectedDevice)}
              >
                <Filter className="mr-1 h-3.5 w-3.5" />
                {onlyConnectedDevice ? "Scope: Target Only" : "Scope: All Traffic"}
              </Button>
            )}
        ...
        <CardContent className="pt-1">
          <div className="max-h-[420px] space-y-2 overflow-y-auto pr-1">
            {loading ? (
              ...
              <div className="py-8 text-center text-xs space-y-2 text-slate-500 dark:text-slate-400">
                {deviceFilterState === "no_identity" ? (
                  <div className="max-w-md mx-auto rounded-xl border border-amber-500/30 bg-amber-500/10 p-4 text-center dark:border-amber-500/40">
                    <AlertTriangle className="mx-auto h-5 w-5 text-amber-600 dark:text-amber-400 mb-1.5" />
                    <p className="text-xs font-semibold text-amber-800 dark:text-amber-200">
                      Device Identity Unavailable
                    </p>
                    <p className="text-[11px] text-amber-700/80 dark:text-amber-300/80 mt-1">
                      Unable to filter packets because the connected device has no IP identity.
                    </p>
                  </div>
                ) : connectedDevice && onlyConnectedDevice ? (
                  <div className="max-w-md mx-auto rounded-xl border border-slate-200 bg-slate-50/80 p-4 dark:border-slate-800 dark:bg-slate-850">
                    <div className="flex items-center justify-center gap-2 text-slate-700 dark:text-slate-200 font-semibold mb-1">
                      <Laptop className="h-4 w-4 text-indigo-500" />
                      <span>{connectedDevice.name} ({connectedDevice.ip || connectedDevice.ip_address || "No IP"})</span>
                    </div>
                    <Badge variant="outline" className="bg-amber-500/10 text-amber-600 border-amber-500/30 text-[10px] mb-2 font-mono">
                      CONNECTED / DISCOVERED • NO TRAFFIC OBSERVED
                    </Badge>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400">
                      <strong>Security Status: NOT ASSESSED</strong> — Insufficient traffic data for malware assessment.
                    </p>
                    <p className="text-[10px] text-slate-400 dark:text-slate-500 mt-1">
                      Generate network activity on this device or click <strong>Scope: All Traffic</strong> to view background packets.
                    </p>
                  </div>
                ) : searchFilter ? (
              ...
              filteredPackets.slice(0, 15).map((packet, index) => {
                ...
                return (
                  <div
                    key={`pkt-${packet.timestamp || index}-${packet.source_ip || "src"}-${packet.dest_ip || "dst"}-${index}`}
                  ...
                  <div className="space-y-3 pt-1">
                    ...
                    <div className="space-y-2">
                      {deduplicateBy(
                        ...
                        (f, i) => f.feature_name ? `${f.feature_name}-${f.rank || i}` : `feat-${i}`,
                        "PacketInspectionPanel.features"
                      ).map((feat, idx) => {
                        const isRisk = feat.direction === "+"
                        const pctValue = parseFloat(feat.contribution_percent.replace("%", "")) || 10
                        const featKey = createEntityKey(
                          "pkt-feat",
                          feat.feature_name || `feat-${idx}`,
                          [String(feat.rank ?? idx), feat.contribution_percent, String(idx)]
                        )
                        return (
                          <div
                            key={featKey}
  ...
  useEffect(() => {
    const fetchAnalysis = async () => {
      try {
        ...
        console.warn("Traffic analysis unavailable:", err instanceof Error ? err.message : String(err))
      } finally {
        setLoading(false)
      }
    }
