...
export default function WorkbenchPage() {
  ...
  const forecastData = useMemo(() => {
    if (!analysisResult?.forecast) return []
    const current = {
      step: "Current (t)",
      probability: currentProb,
      isCurrent: true,
    }
    const steps = (analysisResult.forecast.forecast || []).map((item) => ({
      step: `t+${item.step}`,
      probability: Math.round(item.probability * 100),
      isCurrent: false,
    }))
    return [current, ...steps]
  }, [analysisResult, currentProb])
            ...
            <Card className="lg:col-span-2">
              ...
              <CardContent>
                <div className="h-64 w-full">
                  {isMounted && forecastData.length > 0 ? (
                    <ResponsiveContainer width="100%" height="100%" minHeight={240}>
                      ...
                      >
                        ...
                        />
                        <Tooltip
                          content={<CustomChartTooltip valueSuffix="%" title="Infiltration Trajectory" />}
                        />
                        <Area
                          type="monotone"
                          dataKey="probability"
                    ...
                    <div className="flex h-full items-center justify-center text-xs text-muted-foreground">
                      No forecast trajectory available.
                    </div>
                  )}
                </div>
                ...
                <div className="mt-4 grid grid-cols-3 sm:grid-cols-6 gap-2 text-center text-xs">
                  {forecastData.map((item, i) => (
                    <div key={i} className="rounded-lg border bg-muted/40 p-2 text-center flex flex-col justify-between">
                      <div className="font-semibold text-[11px] text-muted-foreground truncate">{item.step}</div>
                      <div className="mt-1 text-sm font-bold text-sky-500 font-mono">{item.probability}%</div>
                    </div>
                  ))}
                </div>
              </CardContent>
